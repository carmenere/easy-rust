...
- [Closures](#closures)
  - [Example](#example-2)
- [Threads constraints](#threads-constraints)
...
- [Scoped threads constraints](#scoped-threads-constraints)
...
## `Send` constraint
The `thread::spawn` requires its argument to be `Send` because the **closure** is passed **by value** to the **new** thread.<br>
A **closure** is `Send` if **all** of its captues are.<br>
